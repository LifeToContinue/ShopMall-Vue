...
<script>
import { reqTradeInfoListData } from "@/api";
export default {
  name: "Trade",
  data(){
    return {
...
/**
 * 1. 之前浏览商品或是选择电商的商品都可以不用登陆，不用账号，但是也可以加入购物车
 * 2. 之前是用一个uuid生成的唯一id发送给了服务器
 * 3. 后期有些页面，是分权限的，不是谁都发送请求来查看的，必须登陆，比如订单支付页面
 * 4. 所以像支付页面这样的，必须先登陆，因为登陆成功之后，服务器端会根据每一个登陆的账号生成一个唯一的token 用于确定不同的用户身份
 * 5. 登陆完成之后，每次跳转其它页面都会在请求头中自动携带token
 * 6. 服务器接收到请求之后，就会先从请求头中获取发送过来的token，和自己服务器中的token做个校验
 * 7. 如果校验成功，则允许请求，比如订单支付 
 * 
 * 
 */
